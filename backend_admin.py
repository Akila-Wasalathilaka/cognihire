...
from models import User, Assessment, JobRole, Tenant, CandidateProfile, RoleGamePackage
...
async def get_admin_analytics_overview(
    ...
    # Get total candidates (non-admin users)
    total_candidates = db.query(User).filter(User.role != 'admin').count()
...
async def delete_admin_job_role(
    ...
    # Check if any role game packages use this job role
    role_games_with_role = db.query(RoleGamePackage).filter(RoleGamePackage.job_role_id == job_role_id).count()
    if role_games_with_role > 0:
        raise HTTPException(
            status_code=400, 
            detail=f"Cannot delete job role. {role_games_with_role} game configurations use this role."
        )
